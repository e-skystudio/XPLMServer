...
void GetCallbacks(std::vector<CallbackFunctionStruct*>* callbacks, int* size)
{
	*size = 0;
	if (callbacks != nullptr)
	{
		...
		callbacks->push_back(new CallbackFunctionStruct("OVERRIDE_TCAS", "OverrideTcas"));
		callbacks->push_back(new CallbackFunctionStruct("UPDATE_TRAFFIC", "UpdateTraffic"));
...
int RegisterDataref(json& jdata, CallbackManager& callback)
{
	...
	//return 1;
	return (int)callback.AddNamedDataref(name, dataref);
}
...
int UnregisterDataref(json& jdata, CallbackManager& callback)
{
	...
	return (int)callback.RemovedNamedDataref(name);
}
...
int OverrideTcas(json& jdata, CallbackManager& callbackManager)
{
	auto value = jdata.value("Override", 0);
	if (value)
	{
		if (!XPLMAcquirePlanes(NULL, NULL, NULL))
		{
			jdata["Error"] = "Error While aquiring airplanes.";
			return 0x01;
		}
	}
	else {
	...
	Dataref d;
	d.Load("sim/operation/override/override_TCAS");
	d.SetValue(std::to_string(value));
	return 0;
}

int UpdateTraffic(json& jdata, CallbackManager& callbackManager)
{
